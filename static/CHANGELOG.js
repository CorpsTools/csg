...
window.CHANGELOG_HASH = (JSON.stringify(window.CHANGELOG).split('').reduce((a, v) => a + v.charCodeAt(0), 0) % 2 ** 16).toString(36);

if(localStorage.getItem('lastChangelogHash') === null) localStorage.setItem('lastChangelogHash', '-1');

const lastChangelogHash = localStorage.getItem('lastChangelogHash');
window.NEW_UPDATE = lastChangelogHash !== window.CHANGELOG_HASH;